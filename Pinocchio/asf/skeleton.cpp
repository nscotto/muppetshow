...
namespace asf {
	...
	bool compare_degree(Bone *a, Bone *b)
	{
		return a->deg > b->deg;
	}

	int compare_name(Bone *a, Bone *b)
	{
	...
	int Skeleton::readASFfile(const char* asf_filename, double scale)
	{
		...
		if (is.fail()) {
			cerr << "File error: " << asf_filename << endl;
			return -1;
		}
		...
		lBones.reserve(NUM_BONES_IN_ASF_FILE);
		std::vector<Bone*> rBones;
		rBones.reserve(NUM_BONES_IN_ASF_FILE);
		bonesFeet = std::vector<Bone *>(NUM_BONES_IN_ASF_FILE);
		for (int i = 0; i < NUM_BONES_IN_ASF_FILE; i++) {
			Bone *cur = &m_pBoneList[i];
			cout << cur->name << ": (" << cur->absx << ", " << cur->absy << ", " << cur->absz << ")" << endl;
			Bone *it = cur->sibling;
			while (it) {
				cur->deg++;
				it = it->sibling;
			}
			if (m_pBoneList[i].child)
				cur->deg++;

			if (cur->name[0] == 'l')
				lBones.push_back(cur);
			else if (cur->name[0] == 'r')
				rBones.push_back(cur);
			else
				bonesFat.push_back(cur);

			if (strcmp(&(cur->name[1]), "foot") == 0) {
				bonesFeet.push_back(cur);
			}
			else if (strcmp(&(cur->name[1]), "toes") == 0) {
				bonesFeet.push_back(cur);
			}
		}
		// look for bones pairs
		bonePairs = std::vector<std::tuple<Bone*, Bone*>>();
		std::sort(lBones.begin(), lBones.end(), compare_name);
		std::sort(rBones.begin(), rBones.end(), compare_name);
		for (Bone* l: lBones) {
			int i = 0;
			do {
				int c = strcmp(&(l->name[1]), &(rBones[i]->name[1]));
				if (c == 0) {
					bonePairs.push_back(std::tuple<Bone*, Bone*>(l, rBones[i]));
					rBones.erase(rBones.begin()+i);
					break;
				}
				else if (c > 0) {
					bonesFat.push_back(l);
					break;
				}
		...
		// mark resting bones as fat
		/*
		   for (Bone *r: rBones) {
		   int i = 0;
		   do {
		   int c = strcmp(&(r->name[1]), &(lBones[i]->name[1]));
		   if (c == 0) {
		   break;
		   }
		   else if (c > 0) {
		   cout << "Pushing " << r->name << endl;
		   bonesFat.push_back(r);
		   break;
		   }
		   if (++i == lBones.size()) {
		   cout << "Pushing " << r->name << endl;
		...
		/*
		   ...
		   bonesFat.push_back(l);
		   */
		bonesFat.insert(bonesFat.begin(), rBones.begin(), rBones.end());
	...
	void Skeleton::normalize() {
		...
		for (int i = 0; i < NUM_BONES_IN_ASF_FILE; i++) {
			const Bone *b = &(m_pBoneList[i]);
			double x = b->absx, y = b->absy, z = b->absz;
			if (x < min[0])
				min[0] = x;
			else if (x > max[0])
				max[0] = x;
	...
	void Skeleton::initAbsolutePos(Bone * par)
	{
		Bone *it  = par->child;
		while (it)
		{
			double length = it->length;
			it->absx = par->absx + length*it->dir[0];
			it->absy = par->absy + length*it->dir[1];
			it->absz = par->absz + length*it->dir[2];
			initAbsolutePos(it);
			it = it->sibling;
		}
	}

	void Skeleton::initAbsolutePos()
	{
		initAbsolutePos(m_pRootBone);
	}
	...
	void Skeleton::print() const
	{
		...
		cout << "End of skeleton bones" << endl << endl;
	}

	} // end of namespace asf
