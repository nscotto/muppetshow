...
class PINOCCHIO_API ASFSkeleton : public Skeleton
{
	private:
		asf::Skeleton asf_skel;
		void buildSkeleton(asf::Bone *parent, asf::Bone *child);
		
	public:
		ASFSkeleton(const std::string &filename);
		ASFSkeleton(const char *filename);
		~ASFSkeleton(){}
}; 
