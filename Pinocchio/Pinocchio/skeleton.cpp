...
using asf::Bone;
// ADDED
void ASFSkeleton::buildSkeleton(Bone *parent, Bone *child) {
	if (child) {
		double x = child->dir[0], y = child->dir[1], z = child->dir[2];
		double l = child->length;
		makeJoint(child->name, l*Vector3(x, y, z), parent->name);
		/*
		double x = child->absx, y = child->absy, z = child->absz;
		makeJoint(child->name, Vector3(x, y, z), parent->name);
		*/
		std::cout << parent->name << " <--> " << child->name <<std::endl;
		buildSkeleton(parent, child->sibling);
		buildSkeleton(child, child->child);
	}
}

ASFSkeleton::ASFSkeleton(const std::string &filename) : ASFSkeleton(filename.c_str())
{
	cout << "CONSTRUCTOR1" << endl;
}

ASFSkeleton::ASFSkeleton(const char *filename) 
	: asf_skel(filename, 1.)
{
	asf_skel.print();
	using asf::Bone;
	Bone *root = asf_skel.getRoot();
	double x = root->absx, y = root->absy, z = root->absz;
	makeJoint(root->name, Vector3(x, y, z));
	buildSkeleton(root, root->child);

	vector<tuple<Bone*, Bone*>> pairs = asf_skel.getBonePairs();
	for (auto tup: pairs) {
		auto l = std::get<0>(tup);
		auto r = std::get<1>(tup);
		makeSymmetric(l->name, r->name);
	}
	initCompressed();
	...
	/*
	cout << "PASSED" << endl;
	vector<Bone*> fat = asf_skel.getBonesFat();
	for (Bone *f: fat)
		if (f->deg != 2) {
			cout << "setFat(" << f->name << ")" << endl;
			setFat(f->name);
		}

	vector<Bone*> feet = asf_skel.getBonesFeet();
	for (auto f: feet)
		if (f->deg != 2)
			setFoot(f->name);
			*/
